...
class HandController():
    def __init__(self, node: Node, model: AiNexModel, arm_side: str,
        ...
        # LinearSplineTrajectory
        self.spline = None
        self.spline_duration = 0.0
        self.start_time = None

        # singularity
        self.w_threshold = 3e-4  # manipulability threshold
    ...
    def update(self, dt: float) -> np.ndarray:
        ...
        self.in_singularity = (w < self.w_threshold)
        ...
        if self.ignore_manipulability:
            ...
            if self.in_singularity:
                u = np.zeros_like(u)
                # scale = np.clip(w / self.w_threshold, 0.0, 1.0)
                # u = scale * u
            return u
